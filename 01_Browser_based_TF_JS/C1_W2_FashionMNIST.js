...
async function train(model, data) {
    ...
    const [trainXs, trainYs] = // YOUR CODE HERE
          tf.tidy(()=>{
              const dat = data.nextTrainBatch(TRAIN_DATA_SIZE);
              return [
                  dat.xs.reshape([TRAIN_DATA_SIZE,28,28,1]),
                  dat.labels
              ];
          });
    ...
    const [testXs, testYs] = // YOUR CODE HERE
          tf.tidy(()=>{
              const dat = data.nextTestBatch(TEST_DATA_SIZE);
              return [
                  dat.xs.reshape([TEST_DATA_SIZE,28,28,1]),
                  dat.labels
              ];
          });
